...
def fashionmnist(config, path, param):
    print("START FashionMNIST")
    use_cuda = config.general.use_cuda and torch.cuda.is_available()
    torch.manual_seed(config.general.seed)
    device = torch.device("cuda" if use_cuda else "cpu")
    print("START TRAINING TARGET MODEL")
    data_train_target = custum_FashionMNIST(True, 0, config, '../data', train=True, download=True,
                   transform=transforms.Compose([
                       transforms.ToTensor(),
                       transforms.Normalize((0.1307,), (0.3081,))
                   ]))
    data_test_target = custum_FashionMNIST(True, 0, config, '../data', train=False, transform=transforms.Compose([
                       transforms.ToTensor(),
                       transforms.Normalize((0.1307,), (0.3081,))
                   ]))
    ...
    print("dataset size", dataset_sizes_target)
    model_target = Net_fashionmnist().to(device)
    ...
    for num_model_sahdow in range(config.general.number_shadow_model):
        ...
        data_train_shadow = custum_FashionMNIST(False, num_model_sahdow, config, '../data', train=True, download=True,
                       transform=transforms.Compose([
                           transforms.ToTensor(),
                           transforms.Normalize((0.1307,), (0.3081,))
                       ]))
        data_test_shadow = custum_FashionMNIST(False, num_model_sahdow, config, '../data', train=False, transform=transforms.Compose([
                           transforms.ToTensor(),
                           transforms.Normalize((0.1307,), (0.3081,))
                       ]))
        ...
        print("dataset size", dataset_sizes_shadow)
        model_shadow = Net_fashionmnist().to(device)
