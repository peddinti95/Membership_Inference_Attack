...
def stl10(config, path,param):
    print("START STL10")
    use_cuda = config.general.use_cuda and torch.cuda.is_available()
    torch.manual_seed(config.general.seed)
    device = torch.device("cuda" if use_cuda else "cpu")
    print("START TRAINING TARGET MODEL")
    data_train_target = custum_STL10(True, 0, config, '../data', split='train', download=True,
                   transform=transforms.Compose([
                       transforms.ToTensor(),
                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
                   ]))
    data_test_target = custum_STL10(True, 0, config, '../data', split='test', transform=transforms.Compose([
                       transforms.ToTensor(),
                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
                   ]))
    ...
    model_target = Net_stl10().to(device)
    ...
    for num_model_sahdow in range(config.general.number_shadow_model):
        criterion = nn.CrossEntropyLoss()
        data_train_shadow = custum_STL10(False, num_model_sahdow, config, '../data', split='train', download=True,
                       transform=transforms.Compose([
                           transforms.ToTensor(),
                           transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
                       ]))
        data_test_shadow = custum_STL10(False, num_model_sahdow, config, '../data', split='test', transform=transforms.Compose([
                           transforms.ToTensor(),
                            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
                       ]))
        ...
        model_shadow = Net_stl10().to(device)
