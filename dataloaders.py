import torch
import torchvision
from torchvision.datasets import CIFAR10, CIFAR100, MNIST, KMNIST, FashionMNIST, STL10
import numpy as np
    ...
    def __init__(self, target, num, config, *args, **kwargs):
        ...
        if self.train:
            if target:
                ...
                # TODO: cut th size  config.general.train_target_size
        else:
            if target:
                ...
                # TODO: cut the size config.general.test_target_size
    ...
    def __init__(self, target, num, config, *args, **kwargs):
        ...
        if self.split=='train':
            if target:
                #target train size 0:config.general.train_target_size
                self.data = self.data[:config.general.train_target_size]
                self.labels = self.labels[:config.general.train_target_size]
            else:
                #shadow train size config.general.train_target_size:-1
                self.data = self.data[config.general.train_target_size:]
                self.labels = self.labels[config.general.train_target_size*(num+1):config.general.train_target_size*(num+2)]
                # TODO: cut the size  config.general.train_target_size
        else:
            if target:
                #target test size 0:config.general.test_target_size
                self.data = self.data[:config.general.test_target_size]
                self.labels = self.labels[:config.general.test_target_size]
            else:
                #shadow test size config.general.test_target_size:-1
                self.data = self.data[config.general.test_target_size:]
                self.labels = self.labels[config.general.test_target_size*(num+1):config.general.test_target_size*(num+2)]
                # TODO: cut the size config.general.test_target_size
...
class custum_FashionMNIST(FashionMNIST):
